...
class SideBanner2 extends Component {
  ...
  handleBannerUploads = file => {
    var uploadTask = this.props.firebase.uploadSideBanner2ToStorage(file);
    this.setState({
  ...
  handleBannerSave(uid, downloadUrl) {
    this.props.firebase
      .saveSideBanner2(uid, downloadUrl)
      .then(result => {
    ...
    return (
      <div class="side1-banner-upload">
        <div class="side-banner-upload1">
          <div class="upload">
            <Dropzone
              onDrop={this.onSide2Drop.bind(this)}
              accept="image/*"
              multiple={false}
            >
                  ...
                  <div class="upload-space" {...getRootProps()}>
                    <input {...getInputProps()} />
                    {
                      <span class="upload-text">
                        Click or Drop the top Side Banner 2 Picture Here
                      </span>
                    }
                  </div>
                );
...
export default withFirebase(SideBanner2);
